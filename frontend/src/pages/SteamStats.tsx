...
export default function SteamStats() {
  ...
  const [visibleCounts, setVisibleCounts] = useState({ dev: 20, pub: 20, tag: 20 });
  ...
  useEffect(() => {
    ...
    IsSteamSyncing().then((inProgress: boolean) => {
    ...
    return (
      <div className="flex-1 overflow-y-auto bg-main-bg p-8 flex items-center justify-center">
        <div className="text-center max-w-md bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl">
          <svg className="w-20 h-20 mx-auto mb-6 text-[#66c0f4] drop-shadow-lg" viewBox="0 0 24 24" fill="currentColor">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Steam Not Connected</h2>
          <p className="text-sm text-white/60 mb-6">
            To view your beautiful Steam statistics, please open the Settings panel and connect your Steam account.
          </p>
        </div>
      </div>
    );
  ...
  const renderGameList = (key: string, highlightTextClass: string = "text-emerald-500", highlightBgClass: string = "bg-emerald-500") => {
    ...
    if (isFetching) {
      return (
        <div className="flex items-center justify-center py-6 text-xs text-white/40 animate-pulse">
          <span className="loading loading-spinner loading-sm mr-3"></span> Loading library...
        </div>
      );
    }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-3">
                  <h4 className={`text-sm font-semibold text-white/90 truncate ${highlightTextClass} transition-colors`}>
                    {game.name}
                  </h4>
                ...
                <div className="flex items-center justify-between gap-2 mt-1 text-[11px] text-white/40">
                  <span>
                    {game.playtime2Weeks > 0 ? (
                      <span className={`${highlightTextClass} font-medium tracking-wide`}>{recentHours}h <span className="text-white/30 font-normal">past 2w</span></span>
                    ) : (
                      <span>All-time</span>
                    )}
                  </span>
                  ...
                  {game.achievementsPct > 0 && (
                    <div className="flex items-center gap-1.5">
                      <div className="w-12 h-1 bg-black/40 rounded-full overflow-hidden">
                        <div className={`h-full ${highlightBgClass} transition-all duration-500`} style={{ width: `${game.achievementsPct}%` }} />
                      </div>
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-sm text-white/40 font-medium">Analyze your lifetime library, playtime, and achievements.</p>
            </div>
            <div className="flex items-center gap-4">
              ...
              >
                <svg className={syncing ? "animate-spin" : ""} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
          ...
          {loading ? (
            <div className="flex justify-center items-center h-64">
              <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-sm text-white/40 font-medium animate-pulse">Loading insights...</span>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-10">
              ...
              {overall && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "Total Games", value: overall.totalGames, icon: "M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z", color: "text-sky-400" },
                    { label: "Hours Played", value: `${overall.totalHours}h`, icon: "M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6", color: "text-amber-400" },
                    { label: "Achievements", value: overall.totalAchievements, icon: "M12 15l-3.09 1.63.59-3.45-2.5-2.44 3.47-.5L12 7l1.53 3.24 3.47.5-2.5 2.44.59 3.45z", color: "text-emerald-400" },
                    { 
                      label: "Completion", 
                      ...
                      progress: overall.totalAchievements > 0 ? (overall.unlockedAchievements / overall.totalAchievements) * 100 : 0
                    }
                  ].map((stat, i) => (
                    <div key={i} className="group relative bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 hover:border-white/10 rounded-3xl p-6 transition-all duration-300 overflow-hidden">
                      <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={stat.color}>
                          <path d={stat.icon} />
                        </svg>
                      </div>
                ...
                <div className="bg-[#121214] border border-white/5 rounded-3xl p-6 flex flex-col shadow-xl">
                  ...
                  <div className="space-y-2 flex-1 overflow-y-auto max-h-[700px] pr-2 custom-scrollbar">
                    {devs.slice(0, visibleCounts.dev).map((d, i) => {
                      ...
                      const maxVal = sortBy === "games" ? devs[0]?.gamesCount : devs[0]?.totalHours;
                      const val = sortBy === "games" ? d.gamesCount : d.totalHours;
                      const pct = Math.min(100, Math.max(2, (val / (maxVal || 1)) * 100));

                      return (
                        <div 
                          key={i} 
                          className={`p-3 rounded-2xl transition-all duration-300 border ${
                            isExpanded ? "bg-white/[0.03] border-white/10" : "bg-transparent border-transparent hover:bg-white/[0.02] hover:border-white/5"
                          }`}
                        >
                          <div 
                          ...
                          >
                            <div className="flex justify-between items-center text-sm">
                              <div className="flex items-center gap-2.5 min-w-0 pr-3">
                                ...
                                <span className="font-bold text-white/90 truncate group-hover:text-white transition-colors tracking-tight">{d.name}</span>
                              </div>
                              <span className="text-white/40 tabular-nums whitespace-nowrap text-xs font-medium">
                                {sortBy === "hours" ? (
                                  <><strong className="text-white/90">{d.totalHours}h</strong> • {d.gamesCount}g</>
                                ) : (
                                  <><strong className="text-white/90">{d.gamesCount}g</strong> • {d.totalHours}h</>
                                )}
                              </span>
                            </div>
                            
                            <div className="w-full bg-black/40 h-1.5 rounded-full overflow-hidden ml-6 w-[calc(100%-24px)]">
                              <div 
                          ...
                          {isExpanded && renderGameList(key, "text-sky-400 group-hover:text-sky-400", "bg-sky-400")}
                        </div>
                      );
                    })}
                    {devs.length === 0 && <p className="text-sm text-white/30 py-8 text-center font-medium">No data available.</p>}
                ...
                <div className="bg-[#121214] border border-white/5 rounded-3xl p-6 flex flex-col shadow-xl">
                  ...
                  <div className="space-y-2 flex-1 overflow-y-auto max-h-[700px] pr-2 custom-scrollbar">
                    {pubs.slice(0, visibleCounts.pub).map((p, i) => {
                      ...
                      const isExpanded = !!expandedKeys[key];
                      const maxVal = sortBy === "games" ? pubs[0]?.gamesCount : pubs[0]?.totalHours;
                      const val = sortBy === "games" ? p.gamesCount : p.totalHours;
                      const pct = Math.min(100, Math.max(2, (val / (maxVal || 1)) * 100));
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div className="flex justify-between items-center text-sm">
                              <div className="flex items-center gap-2.5 min-w-0 pr-3">
                                ...
                                <span className="font-bold text-white/90 truncate group-hover:text-white transition-colors tracking-tight">{p.name}</span>
                              </div>
                              <span className="text-white/40 tabular-nums whitespace-nowrap text-xs font-medium">
                                {sortBy === "hours" ? (
                                  <><strong className="text-white/90">{p.totalHours}h</strong> • {p.gamesCount}g</>
                                ) : (
                                  <><strong className="text-white/90">{p.gamesCount}g</strong> • {p.totalHours}h</>
                                )}
                              </span>
                            </div>
                          ...
                          {isExpanded && renderGameList(key, "text-amber-400 group-hover:text-amber-400", "bg-amber-400")}
                        </div>
                      );
                    })}
                    {pubs.length === 0 && <p className="text-sm text-white/30 py-8 text-center font-medium">No data available.</p>}
                ...
                <div className="bg-[#121214] border border-white/5 rounded-3xl p-6 flex flex-col shadow-xl">
                  ...
                  <div className="space-y-2 flex-1 overflow-y-auto max-h-[700px] pr-2 custom-scrollbar">
                    {tags.slice(0, visibleCounts.tag).map((t, i) => {
                      const key = `tag:${t.tag}`;
                      const isExpanded = !!expandedKeys[key];
                      const maxVal = sortBy === "games" ? tags[0]?.gamesCount : tags[0]?.totalHours;
                      const val = sortBy === "games" ? t.gamesCount : t.totalHours;
                      const pct = Math.min(100, Math.max(2, (val / (maxVal || 1)) * 100));
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div className="flex justify-between items-center text-sm">
                              <div className="flex items-center gap-2.5 min-w-0 pr-3">
                                ...
                                <span className="font-bold text-white/90 truncate group-hover:text-white transition-colors tracking-tight">{t.tag}</span>
                              </div>
                              <span className="text-white/40 tabular-nums whitespace-nowrap text-xs font-medium">
                                {sortBy === "hours" ? (
                                  <><strong className="text-white/90">{t.totalHours}h</strong> • {t.gamesCount}g</>
                                ) : (
                                  <><strong className="text-white/90">{t.gamesCount}g</strong> • {t.totalHours}h</>
                                )}
                              </span>
                            </div>
                          ...
                          {isExpanded && renderGameList(key, "text-indigo-400 group-hover:text-indigo-400", "bg-indigo-400")}
                        </div>
                      );
                    })}
                    {tags.length === 0 && <p className="text-sm text-white/30 py-8 text-center font-medium">No data available.</p>}
