...
export default function ChannelPage() {
  ...
  // Player view: chronological order (oldest → newest)
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {selectedPlaylist ? (
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col cursor-pointer group" onClick={() => setViewType("grid")} title="Back to playlist grid">
                  <span className="text-[10px] font-bold text-accent leading-none mb-1 group-hover:text-accent/80 transition-colors">
                    Playlist
                  </span>
            ...
            {!selectedPlaylist && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-text-muted">View:</span>
                <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-elevated/50 p-1 rounded-lg border border-border-subtle">
              <button onClick={() => setViewMode("grid")} className={`p-1 rounded transition-all ${viewMode === "grid" ? "bg-accent text-white shadow-sm" : "text-text-muted hover:text-text-primary"}`} title="Grid View">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" />
                </svg>
              </button>
              <button onClick={() => setViewMode("list")} className={`p-1 rounded transition-all ${viewMode === "list" ? "bg-accent text-white shadow-sm" : "text-text-muted hover:text-text-primary"}`} title="List View">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {viewType === "player" && selectedVideo ? (
        <div className="flex-1 flex overflow-hidden">
          ...
          <aside className="w-[420px] flex flex-col border-r border-border-subtle bg-surface shrink-0 z-10">
            <div ref={sidebarRef} className="flex-1 overflow-y-auto overflow-x-hidden p-3 flex flex-col gap-3 custom-scrollbar">
              {playerVideos.map((v) => (
            ...
            videos.length === 0 ? (
              ...
              <div className={viewMode === "grid" ? "grid grid-cols-[repeat(auto-fill,213px)] justify-center gap-6" : "flex flex-col gap-3"}>
                ...
                <div ref={loadMoreRef} className="col-span-full h-20 flex items-center justify-center">
                  {loadingMore && <div className="w-6 h-6 border-2 border-accent border-t-transparent rounded-full animate-spin" />}
                </div>
              </div>
            )
          ) : playlists.length === 0 ? (
