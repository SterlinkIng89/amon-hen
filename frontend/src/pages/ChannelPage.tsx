...
export default function ChannelPage() {
  ...
  const [activeSyncType, setActiveSyncType] = useState<"light" | "full" | null>(
    null,
  );
  ...
  useEffect(() => {
    localStorage.setItem("ch:playlistSort_v2", playlistSort);
  }, [playlistSort]);
  useEffect(() => {
    localStorage.setItem("ch:videoSort", videoSort);
  }, [videoSort]);
      ...
      (
        ...
        if (typeof data === "string") {
          ...
          if (
            ...
            setSyncStatus(
              `Syncing: ${data.stage} (${data.count}/${data.total})`,
            );
          } else if (data.stage) {
  ...
  const handleSyncFull = () =>
    runSync(
      "full",
      "Full sync — fetching entire channel...",
      () => SyncChannelData(),
      "Full sync failed:",
    );
  ...
  const handleNext = () => {
    ...
    if (idx !== -1 && idx < playerVideos.length - 1)
      setSelectedVideo(playerVideos[idx + 1]);
  };
  ...
  const handlePlaylistClick = (playlist: YTPlaylist, e: React.MouseEvent) => {
    ...
    if (e.shiftKey) {
      ...
      e.stopPropagation();
      const anchorId =
        lastSelectedPlaylistId.current || visiblePlaylists[0]?.id;
      const currentIndex = visiblePlaylists.findIndex(
        (p) => p.id === playlist.id,
      );
  ...
  const handleUpdatePlaylistsVisibility = async (
    ...
    setSyncStatus(
      `Updating ${targetIds.length} playlist${targetIds.length !== 1 ? "s" : ""} to ${privacyLabel}...`,
    );

    try {
      await UpdatePlaylistsVisibility(targetIds, privacy);
      setSyncStatus(
        `Updated ${targetIds.length} playlist${targetIds.length !== 1 ? "s" : ""} to ${privacyLabel} ✓`,
      );
      setSelectedPlaylistIds(new Set());
      lastSelectedPlaylistId.current = null;
      setTimeout(() => setSyncStatus(""), 4000);
  ...
  const ytGroups =
    activeTab === "videos" &&
    !selectedPlaylist &&
    (videoSort === "recent" || videoSort === "title_date")
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-base">
        ...
        <div className="flex items-center gap-3 px-5 min-h-14 py-2 shrink-0 flex-wrap">
          ...
          {selectedPlaylist ? (
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                <span className="text-[9px] font-bold text-accent leading-none mb-0.5 group-hover:text-accent/80 transition-colors">
                  Playlist
                </span>
              ...
              {duplicateEntriesCount > 0 && (
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-semibold shrink-0">
                  ...
                  <span>
                    {duplicateEntriesCount} duplicate video
                    {duplicateEntriesCount !== 1 ? "s" : ""} detected
                  </span>
                </div>
              )}
            ...
            >
              ...
              <h1
                className={`text-sm font-bold text-text-primary ${viewType === "player" ? "group-hover:text-accent transition-colors" : ""}`}
              >
                Channel
              </h1>
            </div>
          ...
          <ActiveFilterChips
            ...
            analyticsDate={analyticsDate}
            onClearDateRange={() =>
              filters.onChange({ ...filters.value, dateFrom: "", dateTo: "" })
            }
            onClearAnalyticsDate={() => setAnalyticsDate("")}
            onClearExcludeWords={() =>
              filters.onChange({ ...filters.value, excludeWords: [] })
            }
          />
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            ...
            <button
              ...
              title={
                showAnalytics ? "Hide analytics" : "Show channel analytics"
              }
            >
              <svg
                width="13"
            ...
            <div className="flex items-center gap-0.5 bg-elevated/50 p-0.5 rounded-lg border border-border-subtle">
              <button
                ...
                className={`p-1.5 rounded transition-all ${viewMode === "grid" ? "bg-accent text-white shadow-sm" : "text-text-muted hover:text-text-primary"}`}
                title="Grid view"
              >
                <svg
                  width="13"
              ...
              <button
                ...
                className={`p-1.5 rounded transition-all ${viewMode === "list" ? "bg-accent text-white shadow-sm" : "text-text-muted hover:text-text-primary"}`}
                title="List view"
              >
                <svg
                  width="13"
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (activeTab === "videos" || selectedPlaylist)
                  setVideoSort(val);
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-elevated/30 border border-border-subtle rounded-lg">
              ...
              >
                <div
                  className={`absolute top-[1px] w-2.5 h-2.5 rounded-full transition-all ${autoplay ? "right-[1.5px] bg-white" : "left-[1.5px] bg-text-muted"}`}
                />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              {syncStatus && (
                <span
                  className={`text-[10px] font-bold ${isSyncing ? "text-accent animate-pulse" : "text-green-500"} max-w-[140px] truncate`}
                >
                  {syncStatus}
                </span>
              )}
              <div ref={syncMenuRef} className="relative flex items-center">
                ...
                >
                  <svg
                    className={`${isSyncing ? "animate-spin" : "group-hover:rotate-180 transition-transform duration-500"}`}
                    width="13"
                    height="13"
                    viewBox="0 0 24 24"
                    fill="none"
      ...
      {viewType === "player" && selectedVideo ? (
        <div className="flex-1 flex overflow-hidden">
          <aside className="w-[360px] min-w-[280px] max-w-[460px] flex flex-col border-r border-border-subtle bg-surface shrink-0 z-10">
            ...
            <div
              ref={sidebarRef}
              className="flex-1 overflow-y-auto overflow-x-hidden p-2 flex flex-col gap-2 custom-scrollbar"
            >
              {loading ? (
                ...
                <>
                  {playerVideos.map((v, idx) => (
                    <div
                      key={`${v.id}-${idx}`}
                      data-selected={selectedVideo?.id === v.id}
                    >
                      <VideoPill
                        video={v}
          ...
          {loading ? (
              ...
              >
                {filteredVideos.map((v, idx) => (
                  ...
                  >
                    <VideoPill
                      video={v}
                      multiSelected={selectedVideoIds.has(v.id)}
                      onUpdate={() => loadData(true)}
                      viewMode={viewMode}
            ...
            filteredVideos.length === 0 ? (
              ...
              <div className="flex flex-col">
                {viewMode === "grid" && ytGroups.length > 0 ? (
                  ...
                  >
                    {filteredVideos.map((v) => (
                      <div key={v.id} onClick={(e) => handleVideoClick(v, e)}>
                        <VideoPill
                          video={v}
                          multiSelected={selectedVideoIds.has(v.id)}
                          onUpdate={() => loadData(true)}
                ...
                <div
                  ref={loadMoreRef}
                  className="col-span-full h-12 flex items-center justify-center shrink-0 mt-4"
                >
                  {loadingMore && (
                    <div className="w-5 h-5 border-2 border-accent border-t-transparent rounded-full animate-spin" />
                  )}
      ...
      {duplicateWarning && (
        <DuplicateWarningDialog
          ...
          onAddAll={() => {
            processAddVideos(
              duplicateWarning.playlistId,
              duplicateWarning.targetVideoIds,
              [],
            );
          }}
          onAddNewOnly={() => {
