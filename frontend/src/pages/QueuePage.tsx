...
export default function QueuePage({ queue, running, onUpdateQueue, onSetRunning, onStart }: QueuePageProps) {
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden bg-base">
      ...
      {/* ── Dev Simulator ── */}
      <DevSimulator queue={queue} onUpdateQueue={onUpdateQueue} onSetRunning={onSetRunning} />
    </div>
  );
}
...
const MOCK_TITLES = [
  "Halo: Campaign Evolved - 29/07/26 - 4",
  "Halo: Campaign Evolved - 28/07/26 - 3",
  "Halo: Campaign Evolved - 28/07/26 - 2",
  "Halo: CE - 27/07/26 - 1",
  "Halo: CE - 26/07/26 - 5",
];
const MOCK_SIZES = [12683305984, 11491858841, 21320876032, 9023775744, 13000000000];

function makeMockItem(i: number, status: QueueItem["status"] = "pending"): QueueItem {
  return {
    id: crypto.randomUUID(),
    videoPath: `/fake/path/video${i}.mp4`,
    videoName: `video${i}.mp4`,
    size: MOCK_SIZES[i % MOCK_SIZES.length],
    title: MOCK_TITLES[i % MOCK_TITLES.length],
    description: "Gameplay session recording",
    privacy: "unlisted",
    status,
    progress: status === "done" ? 100 : status === "uploading" ? 30 + i * 10 : 0,
    uploadSpeed: status === "uploading" ? 9_000_000 + i * 2_000_000 : undefined,
    startedAt: status === "uploading" || status === "done" ? Date.now() - 120_000 : undefined,
    completedAt: status === "done" ? Date.now() - 10_000 : undefined,
    url: status === "done" ? `https://youtu.be/sim${i}` : undefined,
    error: status === "error" ? "Daily YouTube upload limit reached (~6 videos/day)" : undefined,
    gameTag: "Halo",
  };
}

function DevSimulator({
  queue, onUpdateQueue, onSetRunning,
}: { queue: QueueItem[]; onUpdateQueue: (q: QueueItem[]) => void; onSetRunning: (r: boolean) => void }) {
  const [open, setOpen] = useState(false);
  const tickRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const queueRef = useRef(queue);
  queueRef.current = queue;

  // Stop tick interval on unmount
      ...
      onUpdateQueue(
        queueRef.current.map(item =>
          item.status === "uploading"
            ? {
                ...item,
                progress: Math.min(100, (item.progress || 0) + 3),
                uploadSpeed: 8_000_000 + Math.random() * 6_000_000,
              }
            : item,
        ),
      );
  ...
  const actions: { label: string; color?: string; fn: () => void }[] = [
    {
      label: "+ 3 Pending",
      fn: () => {
        const next = [0, 1, 2].map(i => makeMockItem(i, "pending"));
        onUpdateQueue([...queueRef.current, ...next]);
      },
    },
    {
      ...
      fn: () => {
        ...
        onUpdateQueue(
          queueRef.current.map(item =>
            item.status === "pending"
              ? { ...item, status: "uploading", progress: 5, startedAt: Date.now(), uploadSpeed: 10_000_000 }
              : item,
          ),
        );
        startTick();
    ...
    },
    {
      ...
      fn: () => {
        stopTick();
        const first = queueRef.current.find(i => i.status === "uploading");
        if (!first) return;
        onUpdateQueue(
          queueRef.current.map(i =>
            i.id === first.id ? { ...i, progress: 100, uploadSpeed: 0 } : i,
          ),
        );
      },
    },
    {
      ...
      fn: () => {
        ...
        onUpdateQueue(
          queueRef.current.map(i =>
            i.id === first.id
              ? { ...i, status: "done", progress: 100, uploadSpeed: undefined, completedAt: now, url: "https://youtu.be/simDone" }
              : i,
          ),
        );
      },
        ...
        onUpdateQueue(
          queueRef.current.map(i =>
            i.status === "uploading" || i.status === "pending"
              ? { ...i, status: "done", progress: 100, uploadSpeed: undefined, completedAt: now, startedAt: i.startedAt || now - 300_000, url: "https://youtu.be/simAll" }
              : i,
          ),
        );
        onSetRunning(false);
    ...
    },
    {
      ...
      fn: () => {
        stopTick();
        const first = queueRef.current.find(i => i.status === "uploading" || i.status === "pending");
        if (!first) return;
        onUpdateQueue(
          queueRef.current.map(i =>
            i.id === first.id
              ? { ...i, status: "error", error: "Daily YouTube upload limit reached (~6 videos/day)" }
              : i,
          ),
        );
      },
    },
    {
      label: "🗑 Clear queue",
      color: "#f87171",
      fn: () => { stopTick(); onUpdateQueue([]); onSetRunning(false); },
    },
  ];
  ...
  return (
    <div style={{ position: "fixed", bottom: 16, right: 16, zIndex: 9999, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
      {open && (
        <div style={{
          background: "#18181b", border: "1px solid rgba(249,115,22,0.4)",
          borderRadius: 12, padding: "10px 12px",
          display: "flex", flexDirection: "column", gap: 5, minWidth: 180,
          boxShadow: "0 8px 32px rgba(0,0,0,0.6)",
        }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: "#f97316", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>⚡ Queue Simulator</div>
          {actions.map(a => (
            <button
              key={a.label}
              onClick={a.fn}
              style={{
                ...
                borderRadius: 6, color: a.color || "#f4f4f5",
      ...
      <button
        onClick={() => setOpen(o => !o)}
        title="Dev Queue Simulator"
        style={{
          width: 36, height: 36, borderRadius: "50%",
          background: open ? "#f97316" : "#18181b",
