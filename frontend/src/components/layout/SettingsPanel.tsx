...
export default function SettingsPanel({ open, onClose }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 right-0 h-full w-[380px] max-w-[90vw] bg-surface border-l border-border-subtle shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-50 transform transition-transform duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="flex items-center justify-between p-5 border-b border-border-subtle shrink-0">
          ...
          <button
            className="bg-transparent border-none text-text-secondary cursor-pointer p-1 rounded-sm hover:text-text-primary hover:bg-elevated transition-colors"
            onClick={onClose}
            title="Close"
          >
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6">
              ...
              <div className="flex items-center justify-between gap-3 p-3 bg-elevated border border-border-subtle rounded-md">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-medium text-xs text-text-primary mb-0.5">
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between gap-3 p-3 bg-elevated border border-border-subtle rounded-md">
              <div>
                ...
                <p className="text-[10px] text-text-secondary mt-0.5">
                  Start Amon-Hen automatically when you log in
                </p>
              </div>
              <button
                role="switch"
            ...
            <div className="flex items-center justify-between gap-3 p-3 bg-elevated border border-border-subtle rounded-md">
              <div>
                <p className="text-xs font-medium text-text-primary">
                  Watch folders for new videos
                </p>
                <p className="text-[10px] text-text-secondary mt-0.5">
                  Auto-refresh library when new video files are detected
                </p>
              </div>
              <button
                role="switch"
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 p-3 bg-elevated border border-border-subtle rounded-md">
              ...
              <p className="text-[10px] text-text-secondary mt-0.5">
                Used between segments in auto-generated titles (e.g. Game
              ...
              <div className="bg-elevated border border-accent/30 rounded-md p-3 flex flex-col gap-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm((prev) => ({
                      ...prev,
                      profile: { ...prev.profile, type: e.target.value },
                    }))
                  }
                >
                  <option value="singleplayer">
                    Singleplayer (Episode counter)
                  </option>
                  <option value="multiplayer">
                    Multiplayer (Custom template)
                  </option>
                </select>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setGameProfiles((prev) => ({
                          ...prev,
                          [editForm.tag.trim()]: finalProfile,
                        }));
                        setEditingTag(null);
                      } catch (e) {
                        console.error(e);
            ...
            <div className="flex flex-col gap-2">
              {Object.entries(gameProfiles).length === 0 ? (
                <p className="text-[10px] text-text-muted text-center py-2">
                  No custom profiles. Games default to Singleplayer.
                </p>
              ) : (
                Object.entries(gameProfiles).map(([tag, prof]) => (
                  <div
                    key={tag}
                    className="flex items-center justify-between p-2 bg-elevated border border-border-subtle rounded-md group"
                  >
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-[10px] text-text-secondary truncate">
                        {prof.type === "multiplayer"
                          ? `Template: ${prof.titleTemplate}`
                          : "Singleplayer Mode"}
                      </span>
                    </div>
