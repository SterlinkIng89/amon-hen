...
export default function BulkActionBar({
  ...
  useEffect(() => {
    if (selectedVideos.length > 0) {
      ...
      const allSameGame =
        firstGame && selectedVideos.every((v) => v.game === firstGame);
  ...
  const handleCreateNewPlaylist = async () => {
    ...
    try {
      ...
      const newId = await GetOrCreatePlaylist(
        title,
        "",
    ...
    <div className="w-full bg-elevated border-b border-border-medium z-40 px-4 py-2 flex items-center justify-between animate-fadeIn shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <select
            className="w-[120px] bg-elevated border border-border-subtle rounded-sm px-2 py-1.5 text-xs text-text-primary outline-none transition-colors hover:border-border-medium focus:border-accent focus:bg-card focus: appearance-none cursor-pointer"
          ...
          <TagInput
            ...
            className={`w-[140px] bg-elevated border border-border-subtle rounded-sm px-2 py-1.5 text-xs text-text-primary outline-none transition-colors hover:border-border-medium focus:border-accent focus:bg-card focus: ${selectedProfileTag !== "" ? "opacity-50 cursor-not-allowed" : ""}`}
                ...
                <div className="absolute top-full left-0 mt-2 w-64 bg-elevated border border-border-medium rounded-md shadow-2xl z-[60] max-h-60 overflow-y-auto custom-scrollbar animate-fadeIn">
                  ...
                  {filteredPlaylists.map((p) => (
                    ...
                    >
                      ...
                      <span className="text-[9px] text-text-muted ml-2 shrink-0">
                        {p.videoCount} videos
                      </span>
                    </button>
                  ))}
                    ...
                    {!creatingNew ? (
                      ...
                      <div className="px-3 py-2 flex items-center gap-2">
                        ...
                        <button
                          ...
                          disabled={
                            creatingNewLoading || !newPlaylistTitle.trim()
                          }
                        >
                          {creatingNewLoading ? (
                  ...
                  {filteredPlaylists.length === 0 && !creatingNew && (
                    <p className="px-3 py-2 text-[10px] text-text-muted text-center">
                      No playlists match
                    </p>
                  )}
                </div>
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2">
            <span className="text-xs text-red-400 font-medium">
              Delete {selectedPaths.length} file
      ...
      {pendingTagForModal && (
        <TagPlaylistModal
          ...
          onSaved={() => {
            ...
            LoadConfig()
              .then((cfg) => setTagPlaylists(cfg.tag_playlists || {}))
              .catch(() => {});
            performSave();
          }}
        />
