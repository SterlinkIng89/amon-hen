...
export function useAdvancedFilters(initial?: Partial<AdvancedFiltersValue>) {
 ...
 return { value, onChange, clearAll, activeCount, dateFrom, dateTo, excludeWords, setDateFrom, setDateTo, setExcludeWords };
}
...
export default function AdvancedFilters({
 ...
 } else if (e.key === "Backspace" && !excludeInput && excludeWords.length > 0) {
 ...
 <span className="text-xs font-black text-text-primary">Advanced Filters</span>
 </div>
 {activeFilterCount > 0 && (
 <button
 onClick={clearAll}
 ...
 <span className="text-xs font-bold text-accent flex-1">{analyticsDate}</span>
 <span className="text-[9px] text-accent/70 font-semibold">from analytics</span>
 <button
 ...
 onClick={() => (document.getElementById("adv-exclude-input") as HTMLInputElement)?.focus()}
 >
 {excludeWords.map((w) => (
 <span
 key={w}
 ...
 {w}
 <button
 onClick={(e) => { e.stopPropagation(); removeExcludeWord(w); }}
 ...
 type="text"
 placeholder={excludeWords.length === 0 ? excludePlaceholder : ""}
 value={excludeInput}
 onChange={(e) => setExcludeInput(e.target.value)}
 onKeyDown={handleExcludeKeyDown}
 onBlur={() => { if (excludeInput.trim()) addExcludeWord(excludeInput); }}
 ...
 Press{" "}
 <kbd className="px-1 py-0.5 bg-elevated border border-border-subtle rounded text-[8px]">Enter</kbd>
 {" "}or{" "}
 <kbd className="px-1 py-0.5 bg-elevated border border-border-subtle rounded text-[8px]">,</kbd>
 {" "}to add each word
 </p>
 </div>
 </>
...
export function ActiveFilterChips({
 ...
 {analyticsDate}
 <button onClick={onClearAnalyticsDate} className="p-0.5 rounded-full hover:bg-accent/20 transition-colors">
 ...
 <button onClick={onClearDateRange} className="p-0.5 rounded-full hover:bg-accent/20 transition-colors">
 <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
 <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
 </svg>
 </button>
 ...
 {excludeWords.length === 1 ? `"${excludeWords[0]}"` : `${excludeWords.length} words`}
 <button onClick={onClearExcludeWords} className="p-0.5 rounded-full hover:bg-red-400/20 transition-colors">
 <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
 <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
 </svg>
 </button>
