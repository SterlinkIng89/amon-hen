...
export default function UploadQueue({
  ...
  const processQueue = (currentQueue: QueueItem[]) => {
    ...
    if (pendingItems.length === 0 && uploadingCount === 0) {
      ...
      if (runningRef.current) {
        LogFrontendEvent(
          "[Queue] Queue completely finished. All uploads done.",
        );
      }
      onSetRunning(false);
      return;
  ...
  useEffect(() => {
    const unsub1 = EventsOn(
      "youtube:progress",
      (data: { path: string; percent: number; speed?: number }) => {
    ...
    const unsub2 = EventsOn(
      ...
      (data: { path: string; url: string }) => {
        ...
        const doneItem = queueRef.current.find(
          (i) => i.videoPath === data.path,
        );
        ...
        if (stillActive === 0) {
          LogFrontendEvent(
            `[Queue] Queue completely finished after '${videoTitle}'.`,
          );
          onSetRunning(false);
        } else {
    ...
    const unsub3 = EventsOn(
      ...
      (data: { path: string; message: string }) => {
        ...
        if (stillActive === 0) {
          LogFrontendEvent(
            "[Queue] Queue completely finished with errors on last item.",
          );
          onSetRunning(false);
        } else {
          LogFrontendEvent(`[Queue] Videos remaining in queue: ${stillActive}`);
  ...
  useEffect(() => {
    if (running) {
      LogFrontendEvent(
        `[Queue] Queue started manually (Total items: ${queueRef.current.length})`,
      );
