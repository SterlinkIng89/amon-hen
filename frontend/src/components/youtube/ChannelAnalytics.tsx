...
function StatCard({
 ...
 <span className={`text-2xl font-black tabular-nums tracking-tight ${accent ? "text-accent" : "text-text-primary"}`}>
...
function UploadSparkline({ data }: { data: MonthlyCount[] }) {
 ...
 return (
 <div key={d.month} className="group/bar flex-1 flex flex-col items-center gap-1 relative">
 {/* Tooltip */}
 ...
 <span className="text-[9px] text-text-muted ml-1">{d.month}</span>
 </div>
 </div>
 <div
 ...
 />
 <span className="text-[8px] text-text-muted tabular-nums">{shortMonth}</span>
 </div>
 );
 })}
 </div>
...
function TopVideoRow({
 ...
 const privColor = privacyColors[video.privacy?.toLowerCase()] ?? "text-text-muted";
 ...
 rank === 1 ? "text-yellow-400" : rank === 2 ? "text-zinc-400" : rank === 3 ? "text-amber-600" : "text-text-muted"
 }`}
 >
 {rank}
 </span>
...
export default function ChannelAnalytics({ refreshKey = 0, onDateFilter, heatmapOnly = false, excludeWords = [] }: ChannelAnalyticsProps) {
 ...
 {[0, 1, 2, 3].map((i) => (
 <div key={i} className="h-24 rounded-xl bg-elevated/50 border border-border-subtle" />
 ))}
 </div>
 ...
 <p className="text-sm font-medium">No data yet — sync your channel first</p>
 </div>
 );
 }
 ...
 <TopVideoRow key={v.id} video={v} rank={i + 1} maxViews={maxViews} />
 ))}
 </div>
 </div>
