...
export default function GamingInsights({ filters }: GamingInsightsProps) {
  ...
  const insights = useMemo(() => {
    ...
    Object.entries(daysMap).forEach(([date, data]) => {
      if (data.count > peakDayVideos.count)
        peakDayVideos = { date, count: data.count };
      if (data.hours > marathonDay.hours)
        marathonDay = { date, hours: data.hours };
    });

    let bestMonth = { month: "", hours: 0 };
    Object.entries(monthMap).forEach(([month, data]) => {
      if (data.hours > bestMonth.hours)
        bestMonth = { month, hours: data.hours };
    });

    let mostActiveYear = { year: "", count: 0 };
    let mostHoursYear = { year: "", hours: 0 };
    Object.entries(yearMap).forEach(([year, data]) => {
      if (data.count > mostActiveYear.count)
        mostActiveYear = { year, count: data.count };
      if (data.hours > mostHoursYear.hours)
        mostHoursYear = { year, hours: data.hours };
    });
      ...
      <div className="bg-elevated/30 border border-border-subtle rounded-xl p-6 flex flex-col gap-6 relative overflow-hidden backdrop-blur-xl">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-xs text-text-muted">
            Patterns and records based on your recordings
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            highlight={insights.currentStreak > 0}
          />
          <StatCard
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-xs font-bold text-text-secondary">
              Days Active (Day of Week)
            </h3>
            <div className="flex items-end gap-2 h-32 mt-2">
              {insights.dayOfWeekCount.map((count, idx) => {
                const heightPercent =
                  count > 0 ? Math.max((count / maxDayCount) * 100, 5) : 0;
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              <RecordRow
                label="Peak Day (Videos)"
                value={insights.peakDayVideos.count.toString()}
              ...
              <RecordRow
                ...
                value={insights.mostActiveYear.year}
                date={`${insights.mostActiveYear.count} videos`}
                reverse
              />
            </div>
          </div>
...
function StatCard({
  ...
  return (
    <div
      className={`flex flex-col gap-1 p-4 rounded-xl border transition-colors ${highlight ? "bg-accent/10 border-accent/30" : "bg-surface/50 border-border-subtle hover:border-border-medium"}`}
    >
      <span className="text-xs font-semibold text-text-muted">{title}</span>
      <span
        className={`text-2xl font-black tracking-tight ${highlight ? "text-accent " : "text-text-primary"}`}
      >
        {value}
      </span>
      <span className="text-[10px] tracking-wider font-bold text-text-secondary">
        {subtitle}
      </span>
    </div>
  );
